...
import {
    Plus, Pencil, Trash2, X, Upload, Loader2, Building2, Hash, Check, AlertTriangle, Phone, Briefcase, Users, Search, ChevronLeft, ChevronRight, Eye,
} from "lucide-react";
...
// ✅ Superadmin'ning o'z companyId sini localStorage dan olish
...
function ConfirmDialog({ company, onConfirm, onCancel, deleting }) {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="w-full max-w-sm rounded-2xl border border-white/[0.08] p-6" style={{ background: "#0f2030" }}>
                ...
                <p className="mb-5 text-sm text-gray-400">
                    <span className="font-semibold text-white">&quot;{company.name}&quot;</span>{" "}kompaniyasini o&apos;chirmoqchimisiz?
                </p>
...
function ImageDropZone({ file, preview, onChange }) {
    ...
    const handleFile = (f) => { if (!f || !f.type.startsWith("image/")) return; onChange(f); };
    return (
        <div>
            <p className="mb-1.5 text-xs font-medium uppercase tracking-wider text-gray-500">Logo</p>
            <div
            ...
            {file && <p className="mt-1.5 flex items-center gap-1.5 text-[11px] text-green-400"><Check size={11} /> {file.name}</p>}
        </div>
    );
}
...
function CompanyDrawer({ company, onClose, onSaved }) {
    ...
    const normalizePermission = (perm) => {
        if (!perm) return "";
        const value = String(perm).toUpperCase();
    ...
    const [form, setForm] = useState({
        ...
        permissions:
            Array.isArray(company?.permissions) && company.permissions.length
                ? company.permissions.map(normalizePermission)
                : ["CRM"],
        logo: null,
    });
    ...
    const togglePermission = (perm) => {
        setForm((prev) => {
            const normalizedPermissions = (prev.permissions || []).map(normalizePermission);
            const exists = normalizedPermissions.includes(perm);

            return {
                ...prev,
                permissions: exists
                    ? normalizedPermissions.filter((p) => p !== perm)
                    : [...normalizedPermissions, perm],
            };
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (Array.isArray(form.permissions)) {
                form.permissions.forEach((p) => formData.append("permissions", p));
            }
    ...
    return (
        <div className="fixed inset-0 z-40 flex justify-end" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
            <div className="absolute inset-0" onClick={onClose} />
            <div className="relative flex h-full w-full max-w-md flex-col border-l border-white/6 shadow-2xl" style={{ background: "#071828", animation: "slideIn .25s ease" }}>
                <div className="flex items-center justify-between border-b border-white/6 px-6 py-4">
                    <div>
                        <h2 className="text-base font-semibold text-white">{isEdit ? "Kompaniyani tahrirlash" : "Yangi kompaniya"}</h2>
                        <p className="mt-0.5 text-xs text-gray-600">{isEdit ? `#${company.id}` : "Ma'lumotlarni to'ldiring"}</p>
                    </div>
                    <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-xl border border-white/[0.06] text-gray-500 transition-colors hover:text-white">
                        <X size={15} />
                    </button>
                </div>
...
function CompanyDetailModal({ company, onClose, onEdit }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="absolute inset-0" onClick={onClose} />
            <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-white/[0.08] shadow-2xl"
                style={{ background: "linear-gradient(145deg,#0f2438 0%,#071828 100%)", animation: "modalFade .25s ease" }}>
                <div className="relative h-64 w-full overflow-hidden bg-[#0a1929]">
                    {showLogo ? (
                        <img src={logoUrl} alt={company.name} onError={() => setImgErr(true)} className="h-full w-full object-cover" />
                    ) : (
                        <div className="flex h-full items-center justify-center">
                            <div className="flex h-24 w-24 items-center justify-center rounded-3xl text-3xl font-bold text-blue-300"
                                style={{ background: "rgba(37,99,235,0.15)", border: "1px solid rgba(37,99,235,0.2)" }}>
                                {initials(company.name)}
                            </div>
                        </div>
                    )}
                    <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(7,24,40,0.98) 0%, rgba(7,24,40,0.4) 40%, transparent 100%)" }} />
                        ...
                        {company.permissions?.length > 0 && (
                            <div className="flex flex-wrap gap-1.5">
                                {company.permissions.map((p) => (
                                    <span key={p} className="rounded-lg px-2 py-1 text-[10px] font-semibold"
                                        style={{ background: "rgba(37,99,235,0.16)", color: "#bfdbfe", border: "1px solid rgba(37,99,235,0.28)" }}>{p}</span>
                                ))}
                            </div>
                        )}
                    </div>
                ...
                <div className="grid gap-5 p-6 md:grid-cols-2">
                    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                        ...
                        <div className="space-y-3">
                            <div><p className="text-xs text-gray-500">Kompaniya nomi</p><p className="mt-1 text-sm font-medium text-white">{company.name || "-"}</p></div>
                            <div><p className="text-xs text-gray-500">Manager</p><p className="mt-1 text-sm font-medium text-white">{company.managerName || "-"}</p></div>
                            <div>
                                <p className="text-xs text-gray-500">Telefon</p>
                                {company.phoneNumber ? (
                                    <a href={`tel:${company.phoneNumber}`} className="mt-1 inline-flex items-center gap-2 text-sm font-medium text-green-400 hover:text-green-300">
                                        <Phone size={14} />{company.phoneNumber}
                                    </a>
                                ) : <p className="mt-1 text-sm font-medium text-white">-</p>}
                            </div>
                            <div><p className="text-xs text-gray-500">ID</p><p className="mt-1 text-sm font-medium text-white">{company.id}</p></div>
                        </div>
                    </div>
                    <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                        <div className="mb-3 flex items-center gap-2">
                        ...
                        <div className="space-y-4">
                            <div><p className="text-xs text-gray-500">Tavsif</p><p className="mt-1 text-sm leading-6 text-white">{company.description || "Tavsif mavjud emas"}</p></div>
                            <div>
                                <p className="text-xs text-gray-500">Ruxsatlar</p>
                                <div className="mt-2 flex flex-wrap gap-2">
                                    {company.permissions?.length > 0 ? company.permissions.map((p) => (
                                        <span key={p} className="rounded-lg px-2.5 py-1 text-xs font-medium"
                                            style={{ background: "rgba(37,99,235,0.12)", color: "#93c5fd", border: "1px solid rgba(37,99,235,0.2)" }}>{p}</span>
                ...
                <div className="flex items-center justify-end gap-3 border-t border-white/[0.06] px-6 py-4">
                    <button onClick={onClose} className="rounded-xl border border-white/[0.08] px-4 py-2 text-sm font-medium text-gray-300 transition-colors hover:text-white">Yopish</button>
                    <button onClick={() => onEdit(company)} className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-white"
                        style={{ background: "linear-gradient(135deg,#2563eb,#1d4ed8)" }}>
...
function CompanyCard({ company, onEdit, onDelete, onView, index, canDelete = true }) {
    ...
    return (
        <div onClick={() => onView(company)}
            ...
            <div className="p-4">
                {company.description && <p className="mb-3 line-clamp-2 text-[11px] leading-relaxed text-gray-700">{company.description}</p>}
                <div className="flex flex-wrap gap-1.5">
                    {company.phoneNumber && (
                        <a href={`tel:${company.phoneNumber}`} onClick={(e) => e.stopPropagation()}
                    ...
                    {company.permissions?.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                            {company.permissions.map((p) => (
                                <span key={p} className="rounded-md px-2 py-0.5 text-[10px] font-medium"
                                    style={{ background: "rgba(37,99,235,0.12)", color: "#93c5fd", border: "1px solid rgba(37,99,235,0.2)" }}>{p}</span>
                            ))}
                        </div>
                    )}
...
function CompaniesContent() {
    ...
    return (
        <div className="min-h-screen bg-[#071828]" style={{ fontFamily: "'Segoe UI', sans-serif" }}>
            ...
            <div className="sticky top-0 z-10 border-b border-white/[0.06] bg-[#071828]/90 px-6 py-4 backdrop-blur"
                style={{ animation: "fadeUp .3s ease both" }}>
                <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
                    <div>
                        <h1 className="text-lg font-bold text-white">Kompaniyalar</h1>
                        <p className="mt-0.5 text-xs text-gray-600">{loading ? "Yuklanmoqda..." : `${total} ta kompaniya`}</p>
                    </div>
            ...
            {drawer && (
                <CompanyDrawer company={drawer === "add" ? null : drawer} onClose={() => setDrawer(null)}
                    onSaved={() => {
                        fetchCompanies(page);
            ...
            {delTarget && <ConfirmDialog company={delTarget} onConfirm={handleDelete} onCancel={() => setDelTarget(null)} deleting={deleting} />}
            {selectedCompany && (
